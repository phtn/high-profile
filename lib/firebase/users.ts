 ...
 */
export const createOrUpdateUserInFirestore = async (firestore: Firestore, user: User): Promise<void> => {
  if (!firestore) {
 ...
 */
export const getUserFromFirestore = async (firestore: Firestore, uid: string): Promise<UserData | null> => {
  if (!firestore) {
    throw new Error('Firestore not initialized')
  }
