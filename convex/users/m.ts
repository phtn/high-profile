...
import { addressSchema, contactSchema, preferencesSchema, socialMediaSchema } from './d'
...
export const createOrUpdateUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      ...
      if (args.cashAppUsername !== undefined) updates.cashAppUsername = args.cashAppUsername
...
export const removeAddress = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const addresses = (user.addresses || []).filter((addr) => addr.id !== args.addressId)
...
export const purgeTestUsers = mutation({
  handler: async ({ db }) => {
    ...
    const itemsToDelete = allItems.filter((item) => item.email.startsWith('test'))
